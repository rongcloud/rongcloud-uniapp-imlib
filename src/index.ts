...
function handleSendMessageCallback (callback: SentMessageCallback): string {
  const eventId = Math.random().toString()
  if (callback) {
    const listener = eventEmitter.addListener('rcimlib-send-message', (data: any) => {
      if (data.eventId === eventId) {
...
export function downloadMediaMessage (messageId: number, callback: MediaMessageCallback = {}) {
  const eventId = Math.random().toString()
  const listener = eventEmitter.addListener('rcimlib-download-media-message', (data: any) => {
    if (callback) {
      if (data.eventId === eventId) {
 ...
 * @param callback 回调
 */
export function startCustomerService (kefuId: string, csInfo: CSInfo, callback: CSCallback | null = null) {
  const eventId = Math.random().toString()
  if (callback) {
    const { success, error, modeChanged, selectGroup, pullEvaluation, quit } = callback
    const listener = eventEmitter.addListener('rcimlib-customer-service', (data: any) => {
      if (data.eventId === eventId) {
        if (data.type === 'success') {
