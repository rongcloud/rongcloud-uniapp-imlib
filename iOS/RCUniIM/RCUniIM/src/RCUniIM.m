...
UNI_EXPORT_METHOD_SYNC(@selector(initWithSetup:setup:));
- (void)initWithSetup:(NSString *)key setup:(NSDictionary *)setup {//done
    [self asyncProcessInMainThread:^{
        RCIMIWEngineSetup * engineSetup = nil;
        if(setup!= nil && setup.count> 0){
            engineSetup = [[RCIMIWEngineSetup alloc] init];
            engineSetup.naviServer = setup[@"naviServer"];
            engineSetup.fileServer = setup[@"fileServer"];
            engineSetup.statisticServer = setup[@"statisticServer"];
...
- (RCMessageContent *)toCustomMessageContent:(NSDictionary *) content {
    RCMessageContent *messageContent;
    
    if (nil == [content objectForKey:@"customType"] || nil == [content objectForKey:@"objectName"]
        || nil == [content objectForKey:@"customFields"]) {
        return nil;
    }
    
    id customTypeObject = [content objectForKey:@"customType"];
    if (customTypeObject == nil || [customTypeObject isKindOfClass:[NSNull class]]){
        return nil;
    }
    
    NSString * customMessageTypeString = [NSString stringWithFormat:@"%@", customTypeObject];\
    if (customMessageTypeString.length == 0 || [customMessageTypeString isEqualToString:@""]) {
        return nil;
    }
    
    int customMessageType = [customMessageTypeString intValue];
    NSString *customMessageObjectName = [content objectForKey:@"objectName"];
    NSDictionary *customMessageFileds = [content objectForKey:@"customFields"];
    
    if(customMessageType < 0 || customMessageType > 3
       || customMessageObjectName == nil || customMessageObjectName.length == 0 || [customMessageObjectName isEqualToString:@""]
       || !customMessageFileds.count)
    {
        return nil;
    }
    
    if (customMessageType == 0 ){
        messageContent = [RCIMIWCommandMessage messageWithType:customMessageObjectName fields:customMessageFileds];
    }
    else if(customMessageType == 1){
        messageContent = [RCIMIWStorageMessage messageWithType:customMessageObjectName fields:customMessageFileds];
    }
    else if(customMessageType == 2){
        messageContent = [RCIMIWNormalMessage messageWithType:customMessageObjectName fields:customMessageFileds];
    }
    else if(customMessageType == 3){
        messageContent = [RCIMIWStatusMessage messageWithType:customMessageObjectName fields:customMessageFileds];
    }
...
- (RCMessageContent *)toMessageContent:(NSDictionary *)content {
    ...
    if([content objectForKey:@"customType"] && [content objectForKey:@"objectName"]){
        messageContent = [self toCustomMessageContent: content];
    } else {
